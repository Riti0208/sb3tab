// USB Host Xbox controller — registers itself as an InputDev::Source.
// Supports Xbox 360 (vendor subclass 0x5D) and Xbox One/Series (subclass 0x47).
...
static const char *TAG = "input_xbox";

// Trigger threshold (0-255 raw) above which LT/RT digital flag is set.
static constexpr uint8_t TRIGGER_DIGITAL_THRESHOLD = 128;

// Shared state (written by USB callback, read by InputDev::get()).
static InputDev::State s_state = {};
static SemaphoreHandle_t s_state_mutex = nullptr;

// Debug message buffer (visible on screen via status_msg()).
...
// ============================================================
// Xbox One init sequence (GIP power-on)
// ============================================================
...
static void xbox_poll(InputDev::State *out) {
...
static const char *xbox_status_msg() {
    return s_debug_msg;
}

static const InputDev::Source s_xbox_source = {
    "xbox-usb",
    xbox_connected,
    xbox_poll,
    xbox_status_msg,
...
void input_xbox_init() {
    ...
    InputDev::register_source(&s_xbox_source);
    ESP_LOGI(TAG, "Xbox input source registered, waiting for controller...");
}
