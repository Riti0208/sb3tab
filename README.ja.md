...
| コントローラー | 状態 |
|---|---|
| **Xbox 360** 有線 | ✅ 実機動作確認済み |
| **Sony DualShock 4** (PID 0x05C4 / 0x09CC) | ✅ 実機動作確認済み |
| **Sony DualSense** (PID 0x0CE6) | ⚠️ コード対応済み、実機未検証 |
| 汎用 USB HID ゲームパッド (class 0x03/0x00/0x00) | ⚠️ HID descriptor 解析対応、実機未検証 |
| **Xbox One / Series** 有線 | ❌ GIP の初期パケットのみ実装、認証・capabilities・hello 等の完全ハンドシェイクは未対応 |
| **Nintendo Switch Pro Controller** | ❌ ESP-IDF v5.4.1 USB host stack の enumeration で `spinlock_acquire` assert → クラッシュ。上流の修正待ち |
| **GP2040-CE** ファーム搭載パッド全般（モード問わず） | ❌ 接続すると enumeration 中に必ず再起動する。原因調査中 |
| Xbox Wireless Adapter ドングル | ❌ Microsoft 独自 RF プロトコル、有線のみ対応 |
| Bluetooth ゲームパッド全般 | ❌ ESP32-C6 側に BT スタックが必要 |
