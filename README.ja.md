...
**コード対応済み・実機未検証:** Sony DualSense (PID 0x0CE6)、Xbox One / Series
有線（GIP ハンドシェイク一部のみ）、汎用 USB HID ゲームパッド (interface
class 0x03 / sub 0x00 / proto 0x00)。接続時に HID Report Descriptor を解析
するので「DirectInput / HID」表記の一般的なパッドはほぼ enumerate できる想定。
